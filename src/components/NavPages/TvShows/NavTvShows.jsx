...
const NavTvShows = () => {
    ...
    const searchMovie = async (event) => {
        if(event.key==="Enter"){
            setSearchKeyword("");
            base_url = `https://api.themoviedb.org/3/search/tv?api_key=3b92f42dd5e2d7a9ab0a778973611246&query=${searchKeyword}`
            let {data} = await axios.get(base_url)
  ...
  return (
    ...
    <div className='tvNavTitleBox'>
        <h1 className='navTitle'>Tv Shows</h1>
    ...
    <div className="movieContentContainer">
            <div className="trending">
                {
                    (movies.length === 0) ? <p className='notFound'>Tv Show not found</p> : movies.map((c) => (
                        <MovieContent 
                        key={c.id} 
                        id={c.id} 
